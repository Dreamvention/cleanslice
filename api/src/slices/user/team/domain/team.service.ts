...
import { ITeamGateway, IFilterTeam } from './';
import { ITeamData, ICreateTeamData, IUpdateTeamData } from './team.types';
import { IMetaResponse } from '#core/domain';

@Injectable()
export class TeamService {
  constructor(private readonly teamGateway: ITeamGateway) {}

  async getTeams(filter?: IFilterTeam): Promise<{ data: ITeamData[]; meta: IMetaResponse }> {
    return this.teamGateway.getTeams(filter);
  }

  async getTeamsByUserId(userId: string, filter?: IFilterTeam): Promise<{ data: ITeamData[]; meta: IMetaResponse }> {
    return this.teamGateway.getTeamsByUserId(userId, filter);
  }
  ...
  async getTeam(id: string): Promise<ITeamData> {
    ...
    return this.teamGateway.getTeam(id);
  }
  ...
  async getTeamByCodename(codename: string): Promise<ITeamData> {
    ...
    return this.teamGateway.getTeamByCodename(codename);
  }
  ...
  async getTeamByIdOrCodename(idOrCodename: string): Promise<ITeamData> {
    ...
    if (idOrCodename.length == 41) {
      return this.teamGateway.getTeam(idOrCodename);
    } else {
      //The Codename can not be longer then 20 symbols
      return this.teamGateway.getTeamByCodename(idOrCodename);
    }
  }
  ...
  async createTeam(data: ICreateTeamData): Promise<ITeamData> {
    ...
    return this.teamGateway.createTeam(data);
  }
  ...
  async updateTeam(id: string, data: IUpdateTeamData): Promise<ITeamData> {
    ...
    return this.teamGateway.updateTeam(id, data);
  }
  ...
  async deleteTeam(id: string): Promise<boolean> {
    ...
    return this.teamGateway.deleteTeam(id);
  }
  ...
  async validateCodename(codename) {
    ...
    if (await this.teamGateway.getTeamByCodename(codename)) {
      return {
        valid: false,
