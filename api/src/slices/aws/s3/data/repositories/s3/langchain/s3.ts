...
import { UnstructuredLoader as UnstructuredLoaderDefault } from './unstructured';
...
export class S3Loader extends BaseDocumentLoader {
  ...
  public async load() {
    ...
    try {
      ...
      const file = this._fs.readFileSync(filePath);

      const unstructuredLoader = new this._UnstructuredLoader(filePath, options);

      const docs = await unstructuredLoader.loadFile(file);
